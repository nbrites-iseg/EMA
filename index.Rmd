...
## Fundamental Concepts {#fund-concepts1.1}
...
## Classical types of stochastic processes {#tipos-classicos1.2}

### Processes with independent and stationary increments {#processos-de-incrementos-indep1.2.1}
...
### Real Second-Order Stochastic Process {#real-second-order1.2.3}
...
### Stationary Processes {#stationary-proc1.2.4}
...
(a) Assuming $X$ is a process with independent and stationary increments, show that: 
...
(b) Consider the stochastic process $Y = (Y_t: t \geq 0)$ defined by:
$$
Y(t) = 
\begin{cases}
t, & \text{if } X_t \geq 0, \\
...
### Markov Processes {#markov-proc1.2.7}
...
**Remark:** In the literature, there are alternative definitions of a diffusion process, some of which assume additional or different hypotheses. For example, in terms of proofs, condition~(iv) can be replaced by a stronger condition requiring the existence of higher-order moments,
\[
\lim_{\Delta \to 0^+} E_{s,x}\!\left[\frac{|X_{s+\Delta} - X_s|^{2+\delta}}{\Delta}\right] = 0,
\]
for some $\delta > 0$.
:::
...
## Itô calculus and stochastic differential equations {#itocalc}
...
**Hint:** apply the change of variable \(X(t) = \ln Y(t)\) and solve the resulting SDE.
:::
...
$$
X(t)=\exp\!\left(
   e^{-rt}\ln x_0+ (1-e^{-r t})\big(\ln K-\dfrac{\sigma^2}{2r}\big)  
   + \sigma\int_0^t e^{r (s-t)}\,dW_s
\right).
$$

**Hint:** apply the change of variable $Z(t)=e^{rt}\ln X(t)$ and solve the resulting SDE.
:::
